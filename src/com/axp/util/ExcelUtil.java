...
public class ExcelUtil
{
	...
	*  * @param paramString 文件名
	 */
	public static void exportBigDataExcell(SXSSFWorkbook localHSSFWorkbook,List<String> paramList, List<Object[]> paramList1, HttpServletResponse paramHttpServletResponse, String paramString) {
	    ...
	    try
	    {
	     // SXSSFWorkbook localHSSFWorkbook = new SXSSFWorkbook(1000);
	      ...
	      paramHttpServletResponse.addHeader("Content-Disposition", String.format("attachment;filename*=utf-8'zh_cn'%s.xls", new Object[] { URLEncoder.encode(paramString, "utf-8") }));
	...
	*  * @param dataType 类型标识数组，制定列是数字还是字符。 数组下标表示列，值决定是否数字(1:数字;0:字符)
	 */
	public static void exportBigDataExcell(SXSSFWorkbook Workbook,List<String> titleList, List<Object[]> dataList, HttpServletResponse response, String fileName,int[] dataType) {
	    ServletOutputStream localServletOutputStream = null;
	    try
	    {
	     // SXSSFWorkbook localHSSFWorkbook = new SXSSFWorkbook(1000);
	        ...
	        for (int k = 0; k < arrayOfObject.length; k++){
	        	if(dataType!=null){
	        		if(k<dataType.length&&dataType[k]==NUMBER_TYPE){
	        			createCell(bodystyle, localHSSFRow2, k, arrayOfObject[k] == null ? "" : arrayOfObject[k].toString(),isPreaseNumber(arrayOfObject[k] == null ? "" : arrayOfObject[k].toString())?true:false);	
	      ...
	      localServletOutputStream = response.getOutputStream();
	      response.reset();
	      response.addHeader("Content-Disposition", String.format("attachment;filename*=utf-8'zh_cn'%s.xls", new Object[] { URLEncoder.encode(fileName, "utf-8") }));
	...
	*  * @param localHSSFSheet steel
	 */
	public static void exportBigDataExcell(SXSSFWorkbook workbook,List<String> titleList, List<Object[]> dataList, HttpServletResponse response, String fileName,int[] dataType,CellStyle cellstyle,Sheet sheet)
	{
	    ServletOutputStream localServletOutputStream = null;
	    try
	    {
	     // SXSSFWorkbook localHSSFWorkbook = new SXSSFWorkbook(1000);
	      ...
	      while (i <= j)
	      {
	        ...
	        for (int k = 0; k < arrayOfObject.length; k++){
	        	if(dataType!=null){
	        		if(k<dataType.length&&dataType[k]==NUMBER_TYPE){
	        			createCell(cellstyle, localHSSFRow2, k, arrayOfObject[k] == null ? "" : arrayOfObject[k].toString(),isPreaseNumber(arrayOfObject[k] == null ? "" : arrayOfObject[k].toString())?true:false);	
	        		}else if(k<dataType.length&&dataType[k]==STRING_TYPE){
	        			createCell(cellstyle, localHSSFRow2, k, arrayOfObject[k] == null ? "" : arrayOfObject[k].toString(),false);
	        		}else{
	        			createCell(cellstyle, localHSSFRow2, k, arrayOfObject[k] == null ? "" : arrayOfObject[k].toString(),false);
	        		}
	        	}else{
	...
	private static void createCell2(CellStyle localHSSFCellStyle,Row paramHSSFRow, int paramShort, String paramString,boolean num,Boolean isImg)
	  {
	    Cell localHSSFCell = paramHSSFRow.createCell(paramShort);
	...
	*  * @param num
	 */
	private static void createCellItem(CellStyle localHSSFCellStyle,Row paramHSSFRow, int paramShort, String paramString,boolean num)
	  {
	    Cell localHSSFCell = paramHSSFRow.createCell(paramShort);
	    localHSSFCellStyle.setAlignment((short)6);
	...
	*  * @param dataType 类型标识数组，制定列是数字还是字符。 数组下标表示列，值决定是否数字(1:数字;0:字符)
	...
	public static void ExportFile(SXSSFWorkbook Workbook,List<String> titleList,
			...
			String fileName,int[] dataType,Boolean isImg,byte[] bytes){
	    ServletOutputStream localServletOutputStream = null;
	    try
	    {
	      localServletOutputStream = response.getOutputStream();
	      Sheet localHSSFSheet = Workbook.createSheet();
	      Workbook.setSheetName(0, fileName);
	      Drawing patriarch = localHSSFSheet.createDrawingPatriarch();
	      Row localHSSFRow1 = localHSSFSheet.createRow(0);
	      Row localHSSFRow2 = null;
	      localHSSFSheet.createFreezePane(0, 1);
	      localHSSFRow1.setHeight((short)400);
	      ...
	      j = dataList.size();
	     /* if(isImg){
	    	  localHSSFSheet.setColumnWidth(0, 6000);
		      localHSSFSheet.setColumnWidth(3, 6000);
		      localHSSFSheet.setColumnWidth(4, 9000);
	      }*/
	      localHSSFSheet.setColumnWidth(4, 9000);
	      for(Object[] objs:dataList){
	    	     localHSSFRow2 = localHSSFSheet.createRow(i);
	    	    // localHSSFRow2.setHeight((short)4500);
	    	     
	    	     int count =0;
	    	  for(Object obj:objs){
	    	     	if(dataType!=null){
	    	     		/*if(count==4&&isImg){
	    	     			ByteArrayOutputStream byteArrayOutobj=(ByteArrayOutputStream) obj;

	    	     			 XSSFClientAnchor anchor = new XSSFClientAnchor(0,0, 0, 0, count,
	    	     					i , (short) (count + 1), i + 1);//参数为图片插入在表格的坐标，可以自行查看api研究参数
	    	     					anchor.setAnchorType(0);
	    	     			
	    	     			patriarch.createPicture(anchor, Workbook.addPicture(byteArrayOutobj.toByteArray(), HSSFWorkbook.PICTURE_TYPE_JPEG));
	    	     			continue;
	    	     		} */
	    	     		
		        		if(count<dataType.length&&dataType[count]==NUMBER_TYPE){
		        			createCell(bodystyle, localHSSFRow2, count,obj== null ? "" : obj.toString(),isPreaseNumber(obj== null ? "" : obj.toString())?true:false);	
	      ...
	      localServletOutputStream = response.getOutputStream();
	      response.reset();
	      response.addHeader("Content-Disposition", String.format("attachment;filename*=utf-8'zh_cn'%s.xlsx", new Object[] { URLEncoder.encode(fileName, "utf-8") }));
	      response.setCharacterEncoding("UTF-8");
	      response.setContentType("application/x-msdownload");
	      response.flushBuffer();
	      Workbook.write(localServletOutputStream);
	...
	*  * @param isShowKey    是否显示关联列 不为空
	...
	public static void ExportHasItemFile(SXSSFWorkbook Workbook,List<String> bodytitleList,
			...
			String fileName,int[] dataType,int[] itemdataType,int keyWord,boolean isShowKey){
	    ServletOutputStream localServletOutputStream = null;
	    try
	    {
	      localServletOutputStream = response.getOutputStream();
	      ...
	      int i = 0;
	      j = bodydataList.size();
	      for(Object[] objs:bodydataList){//主表表头构建
	    	  ...
	    	  localHSSFRow2.setHeight((short)400);
	    	  int count =0;
	    	  for(Object obj:objs){//主表信息构建
	    		  if(obj!=null&&!isShowKey){//如果不显示关连列
	          ...
	          if(keyWord<0||keyWord>=objs.length)
	        	  break;
	          for(Object[] itemobjs:itemdataMap.get(objs[keyWord])){//加载子项
	        	  count =0;
	        	  localHSSFRow3 =localHSSFSheet.createRow(i);
	        	  localHSSFRow3.setHeight((short)400);
	        	  i++;
	        	  for(Object obj:itemobjs){//主表信息构建
		    	     	if(itemdataType!=null){
			        		if(count<itemdataType.length&&itemdataType[count]==NUMBER_TYPE){
			        			createCell(bodystyle, localHSSFRow3, count,obj== null ? "" : obj.toString(),isPreaseNumber(obj== null ? "" : obj.toString())?true:false);	
	...
	*  * @return
    ...
    /**
   	...
   	*  * @throws Exception
   	 */
   	public static <T> List<T> ExcelToBaen(String filepath,int startRow,List<String> title,Class<?> paramClass,FileInputStream f) throws Exception{
	...
	*  * @throws Exception
	 */
	public static <T> List<T> ExcelToBaen(String filepath,int startRow,List<String> title,Class<?> paramClass) throws Exception{
	...
	*  * @throws Exception
	...
	private static <T> List<T> GetListByConverBean(List<Object[]> paramList, List<String> paramList1, Class<?> paramClass) throws Exception
	  {
	      ...
	      while (localIterator.hasNext())
	      {
	        ...
	        while (i < j)
	        {
	          ...
	          if ((arrayOfClass[0] == Integer.TYPE) || (arrayOfClass[0] == Integer.class)){
	        	  ...
	        	  localMethod.invoke(localObject1, new Object[] { Character.valueOf(((Character)arrayOfObject[i]).charValue()) });
