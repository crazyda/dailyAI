...
@Service("adverDaysLogService")
public class AdverDaysLogServiceImpl extends BaseServiceImpl implements
		AdverDaysLogService {
	/** 描述信息配置 */
	private Map<Integer, String> descripConfig;
	...
	private AdvertDaysChangeLog swichtData(Map<Integer, String> map,
			AdvertDaysChangeLog logs, boolean flag) {
		for (Integer key : map.keySet()) {
			int advertType = key;
			int dayCount = flag ? -Integer.parseInt(map.get(key)) : Integer
					.parseInt(map.get(key));
			switch (advertType) {
			case CENTERTYPE:
				logs.setCenterDayCounts(dayCount);
				break;
			case CITYTYPE:
				logs.setCityDayCounts(dayCount);
				break;
			case UNIONTYPE:
