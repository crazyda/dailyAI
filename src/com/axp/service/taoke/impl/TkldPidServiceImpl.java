...
@Service("tkldPidService")
public class TkldPidServiceImpl  extends BaseServiceImpl implements TkldPidService{
	...
	public boolean saveTkldPid(HttpServletRequest request) {
		try {
			...
			if(id==null){
				...
				tkldPid = tkldPidDao.findById(id);
			...
			tkldPid.setIsCareerPartner(level==3?1:3);
		
			TkldPid parentTkldPid=new TkldPid();
			parentTkldPid.setId(parentPidId);
			tkldPid.setTkldPid(parentTkldPid);
			
			
			tkldPidDao.saveOrUpdate(tkldPid);
	...
	@Override
	public boolean saveTkldPid2(HttpServletRequest request) {
		try {
			Integer level = Integer.parseInt( request.getParameter("level"));
			String pId = request.getParameter("pId");
			String ldLoginName = request.getParameter("ldLoginName");
			...
			tkldPidDao.save(tkldPid);
				...
				TkldPid tkldModel = tkldPidDao.findById(id);
	...
	public void findTkldPidList(HttpServletRequest request) {
				...
				Integer count=0;
				if(isValid){
					 count= dateBaseDao.findCount(TkldPid.class, queryModel);
				}
		     ...
		     if(isValid){
		    	 list = dateBaseDao.findPageList(TkldPid.class, queryModel, start,end );
		     }
	...
	public TkldPid getTkldPidById(Integer id){
		QueryModel queryModel=new QueryModel();
		queryModel.combPreEquals("isValid", true);
		TkldPid tkldPid = this.tkldPidDao.findById(id);
		return tkldPid;
	}
	...
	public boolean updateDistributePid(Integer userId, Integer daiLiPid,
		...
		try {
			...
			//根据当前userId找到AdminUserId 是否存在
   			List<TkldPid> list = tkldPidDao.findByCon(TkldPid.class, "adminUser.id="+userId+" and isValid=1");
   			if(list.size()>0){ //重复 
			...
			TkldPid tkldPid = tkldPidDao.findById(daiLiPid);
			if(!flag){
				return false;
			}
	...
	public boolean checkPidByAdminUserId(Integer id) {
			List<TkldPid> list = tkldPidDao.findByCon(TkldPid.class, "adminUser.id="+id+" and isValid=1");
	...
	public void findAdminUserSelect(Integer id,HttpServletRequest request) {
		...
		TkldPid tkldPid=tkldPidDao.findById(id);
	...
	public void editAddPid(Integer id, HttpServletRequest request) {
			...
			if(tkldPid.getLevel()!=1){
				List<TkldPid> list=tkldPidDao.findParentTkldPid(tkldPid.getLevel()-1);
				request.setAttribute("list", list);
			}
	...
	public Map<String, String> saveOrUpdatePartnerPid(Integer id, String userPhone,
			...
			if(userList.size()<=0){
				...
				users=userList.get(0);
					...
					int count = dateBaseDao.findCount(TkldPid.class, queryModel);
						if (count > 0) {
	...
	public Map<String, Integer> currentCityIsValid(HttpServletRequest request) {
		...
		Integer count= dateBaseDao.findCount(TkldPid.class, queryModel);
	...
	public void findPidByCondition(HttpServletRequest request) {
		...
		DetachedCriteria criteria=DetachedCriteria.forClass(TkldPid.class,"t");
	...
	public void partnerDetail(Integer id,HttpServletRequest request) {
		...
		param.append("&id="+id);
	     ...
	     int end = pageInfo.getPageSize();
	     pageInfo.setParam(param + "&page=");
		List<TkldPid> list = dateBaseDao.findPageList(TkldPid.class, queryModel, start,end );
	     	  request.setAttribute("list", list);
	 	     request.setAttribute("page", pagestr);
	 	     request.setAttribute("pageFoot", pageInfo.getCommonDefaultPageFootView());
	}
	...
	public boolean upgradeCareer(HttpServletRequest request) {
			...
			//升级 事业合伙人 
			try {
				TkldPid tkldPid = tkldPidDao.findById(Integer.parseInt(id));
