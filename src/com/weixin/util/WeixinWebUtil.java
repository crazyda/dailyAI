...
public class WeixinWebUtil{
	...
	public static String notifySign(Map<String,String> map){
		...
		List<String> keys = WeixinWebUtil.keyOrderByASCII(map);
	...
	public static String setCloseXmlStr(PayParameter parameter) {
		...
		sbf.append("<xml>\n");
		sbf.append("<appid>"+WeixinConfig.gzh_appid+"</appid>\n");
		sbf.append("<mch_id>"+WeixinConfig.gzh_mch_id+"</mch_id>\n");
...
//		}
	...
	public static String getNonceStr(String attach,String body){
		return MD5.getNonceStr("attach="+attach+"&body="+body+"&device_info="+WeixinConfig.device_info+"&mch_id="+WeixinConfig.gzh_mch_id+"&key="+WeixinConfig.gzh_api_key);
	}
	...
	public static String getParameterSign(Object parameter){
		...
		buffer.append("&key=" + WeixinConfig.gzh_api_key);
	...
	public static String sendXmlWithSSL(String urlStr, String xml) {
		...
		try{
		    ...
		    FileInputStream instream = new FileInputStream(new File(WeixinConfig.gzh_SSLPath));
		    try {
	            keyStore.load(instream, WeixinConfig.gzh_mch_id.toCharArray());
	        } finally {
	            instream.close();
	        }
	        // Trust own CA and all self-signed certs
	        SSLContext sslcontext = SSLContexts.custom()
	                .loadKeyMaterial(keyStore, WeixinConfig.gzh_mch_id.toCharArray())
	                .build();
