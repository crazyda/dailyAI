...
(function( $ ) {
    $.fn.extend({
		...
		*/
        diyUpload:function( opt, serverCallBack ) {
			...
			if( opt.fileQueued ) {
				var diyFileQueued = opt.fileQueued;
				delete opt.fileQueued;
			}
			
			if( opt.fileDequeued ) {
				var diyFileDequeued = opt.fileDequeued;
				delete opt.fileDequeued;
			}
	...
	function createBox( $fileInput, file, webUploader ) {
		...
		var $diyUploadHover = $fileBox.children('.viewThumb').on('dblclick',function(){
		});
