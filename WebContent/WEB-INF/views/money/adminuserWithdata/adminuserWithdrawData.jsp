		...
		<div class="main" style="height:auto;">
			...
			<div class="pageColumn">
				<table class="list" style="width:98%;margin:0 auto;">
					...
					<tbody>
						<c:forEach items="${pageResult.result }" var="datalog">
							<tr>
								<td><input type="checkbox" name="code_Value" value="${datalog.id }" /></td>
								<td>${datalog.id }</td>
								<td>${datalog.name }</td>
								<td>${datalog.phone }</td>
								<td>${datalog.code }</td>
								<td>${datalog.cretatetime }</td>
								<td>
									<div align="center">
										<span class="STYLE4"> 
											<span onclick="put('${datalog.id }');" style="cursor: pointer;"> 
												<img src="<%=basePath%>constant/tab/images/22.gif" width="16" height="16" />审核
											</span>
											<script>
												function put(data){
													location.href="<%=basePath%>AdminwithdrawData/reviewPage?LogId="+data;
												}
											</script>
										</span>
									</div>
