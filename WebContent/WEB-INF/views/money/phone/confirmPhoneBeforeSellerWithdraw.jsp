...
<form class="phonenum_form" action="bindPhone">
    <p>手机号：<label>${bindPhone}</label></p>
    <p style="display: none;">手机号：<input type="text" class="txt" name="phoneNumber" value="${bindPhone}"/></p>
    <p>验证码：<input type="text" class="txt" name="securityCode">
        <input type="button" value="获取验证码" onClick="sendMsg()" class="btn01" data-disabled="1"></p>
...
<script type="text/javascript">
    ...
    //异步提交表单；
    $(".sub").click(function () {
        $.post('<%=basePath%>changePhone/confirmCaptchaBeforeSellerWithdraw', $(".phonenum_form").serialize(), function (data) {
            alert(data.message);
            if(data.success) {
                $.post('<%=basePath%>sellerWithdraw/requestWithdraw2',function(dd){
                    alert(dd.message);
                    if(dd.success){
                        location.href='<%=basePath%>'+dd.url;
                    }
                });
            }
        });
