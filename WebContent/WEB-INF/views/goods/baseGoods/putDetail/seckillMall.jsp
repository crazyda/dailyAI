<%@ page language="java" import="java.util.*" pageEncoding="utf-8" %>
<%@ taglib prefix="c" uri="http://java.sun.com/jsp/jstl/core" %>
<li class="spike default">
    <form action="put" id="saveForm" method="post">
        <div class="spike_con default_con">
            <input type=hidden name=mallType value="3"/> <!-- 商城类型 -->
            <input type="hidden" name="pageflag" value="6"><!-- 验证类型   -->
            <input type=hidden name=baseGoodsId value="${baseGoods.id }"/><!-- 基础商品id -->
            <input type="hidden" name="goodsOrder" value="${reGoodsOfSeckillMall.goodsOrder}"><!--此字段用于区分上传的商品是新增还是重新编辑-->
            <div class="title">秒杀</div>
            <div class="list">
                <dl class="stock">
                    <dt><font color="red">*</font>库存及金额</dt>
                    <dd>
                        <div style="display: inline;">
                            库存：<input type="text" placeholder="" name="noStandardRepertory" class="txt" value="${reGoodsOfSeckillMall.noStandardRepertory}">
                            单价：<input type="text" placeholder="" name="noStandardPrice" class="txt" value="${reGoodsOfSeckillMall.noStandardPrice}">
                            <input class="isNoStandard" type="hidden" name="isNoStandard" value="1">
                        </div>

                        <input type="button" value="编辑库存" class="stock_btn editStandard_btn" style="display: none;">
                        <input type="checkbox" checked onclick="clickNoStandard(this);" class="sellerMallNoStandardCheckBox">
                        <span>商品无规格</span>
                    </dd>
                </dl>
                <dl class="time add_time">
                    <dt><font color="red">*</font>商品原价</dt>
                    <dd>
                        <input type="text" placeholder="" name="displayPrice" class="txt" value="${reGoodsOfSeckillMall.displayPrice}">
                    </dd>
                </dl>
                <dl class="time add_time">
                    <dt><font color="red">*</font>每人限购</dt>
                    <dd>
                        <input type="text" placeholder="" name="seckillCountLimit" class="txt" value="${reGoodsOfSeckillMall.seckillCountLimit}">
                    </dd>
                </dl>
                <dl class="status">
                    <dt>状态</dt>
                    ${reGoodsOfSeckillMall.isChecked?"<dd>已审核</dd>":"<dd>未审核</dd>"}
                </dl>
                <dl class="dely_way">
                    <dt><font color="red">*</font>秒杀区域</dt>
                    <dd>
                    <dd>
                        ${reGoodsOfSeckillMall.seckillArea==0?'<input type="radio" name="seckillArea" value="0" checked><span>当地</span>':'<input type="radio" name="seckillArea" value="0"><span>当地</span>'}
                        ${reGoodsOfSeckillMall.seckillArea==1?'<input type="radio" name="seckillArea" value="1" checked><span>全国</span>':'<input type="radio" name="seckillArea" value="1"><span>全国</span>'}
                    </dd>
                    </dd>
                </dl>
                <dl class="dely_way">
                    <dt><font color="red">*</font>配送方式</dt>
                    <dd>
                        ${reGoodsOfSeckillMall.transportationType==1?"<input type=radio name='transportationType' value='1' checked/>":"<input type=radio name='transportationType' value='1'/>"}
                        <span>包邮</span>
                        ${reGoodsOfSeckillMall.transportationType==2?"<input type=radio name='transportationType' value='2' checked/>":"<input type=radio name='transportationType' value='2'/>"}
                        <span style="margin-right:0;">邮费</span>
                        <input type="text" name="transportationPrice" class="txt" value="${reGoodsOfSeckillMall.transportationPrice}">
                        <span>元</span>
                        ${reGoodsOfSeckillMall.transportationType==3?"<input type=radio name='transportationType' value='3' checked/>":"<input type=radio name='transportationType' value='3'/>"}
                        <span>到店消费</span>
                        ${reGoodsOfSeckillMall.transportationType==4?"<input type=radio name='transportationType' value='4' checked/>":"<input type=radio name='transportationType' value='4'/>"}
                        <span>上门自取</span>
                    </dd>
                </dl>
                <dl class="time add_time">
                    <dt><font color="red">*</font>上架时间</dt>
                    <dd>
                        <input placeholder="点击选择时段" class="txt" type=text id=startTime name="addedTime"
                               onClick="WdatePicker({dateFmt:'yyyy-MM-dd HH:mm:ss'});" value="${reGoodsOfSeckillMall.addedTime}"/>
                    </dd>
                </dl>
                <dl class="time shelf_time">
                    <dt><font color="red">*</font>下架时间</dt>
                    <dd>
                        <input placeholder="点击选择时段" class="txt" type=text id=endTime name="shelvesTime"
                               onClick="WdatePicker({dateFmt:'yyyy-MM-dd HH:mm:ss'});" value="${reGoodsOfSeckillMall.shelvesTime}"/>
                    </dd>
                </dl>
                <dl class="status">
                    <dt><font color="red">*</font>秒杀时段</dt>
                    <dd class="seckillTimes">
                        <c:forEach items="${timesList }" var="times">
                            <input type="checkbox" name="times" class="times"
                                   value=${times.id }>&nbsp;${times.startTime }~${times.endTime }&nbsp;&nbsp;
                        </c:forEach>
                    </dd>
                    <script>
                        //用于回显秒杀时间段；
                        $(function () {
                            var a =${reGoodsOfSeckillMall.arrayForReturnBack};
                            var inputs = $(".seckillTimes input");
                            $.each(inputs, function (index, obj) {
                                var v = $(obj).val();
                                for (var i = 0; i < a.length; i++) {
                                    if (v == a[i]) {
                                        $(obj).prop("checked", true);
                                    }

                                }
                            });
                        });
                    </script>
                </dl>
                <dl class="notice">
                    <dt>秒杀须知</dt>
                    <dd>
                        <textarea placeholder="最多可输入125个字符" name="seckillDesc">${reGoodsOfSeckillMall.seckillDesc}</textarea>
                    </dd>
                </dl>
            </div>
        </div>
        <div class="operate">
            <div class="title">操作</div>
            <div class="op_btn">
                <%--<input type="button" value="紧急下架" class="click">--%>
                <input type="button" value="提交修改" class="click" onclick="submitAjax();">
            </div>
        </div>
        <div class="stock_form standardArea">
            <!-- 选择具体属性的区域 -->
            <div class="checkArea">
                <c:forEach items="${firstStandardList}" var="each">
                    <input data-key="${each.id}" value="${each.name}" type="checkbox"
                           onclick="showSellectAndTableHead(this);"/>${each.name}
                </c:forEach>
                ${reGoodsOfSeckillMallFirstStandards}
            </div>

            <!-- 选择属性明细的区域 -->
            <div class="selectArea" style="display: none;">
                <c:forEach items="${firstStandardList}" var="each">
                    <select data-key="${each.id}" data-value="${each.name}" style="display: none;">
                        <c:forEach items="${each.secondStandards}" var="item">
                            <option value="${item.id}">${item.name}</option>
                        </c:forEach>
                    </select>
                </c:forEach>
                <button type="button" onclick="addItem(this,3,3);">添加</button>
            </div>

            <!-- 生成表格的区域 -->
            <div class="tableArea" style="display: none;">
                <table id="table">
                    <thead>
                    <tr id="tableProperty" class="headLine">
                        ${reGoodsOfSeckillMallTableHead}
                        <th>库存</th>
                        <th>售价</th>
                        <th>操作</th>
                    </tr>
                    </thead>
                    <tbody id="newbody">
                    ${reGoodsOfSeckillMallTableLine}
                    </tbody>
                </table>
            </div>

            <div class="btm">
                <div class="btm_in">
                    <input type="button" value="取消">
                    <input type="button" value="保存" class="click">
                </div>
            </div>
        </div>
    </form>
</li>