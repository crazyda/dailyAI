            ...
            <div class="r_con_wrap">
                <div class="r_con_form">
                    ...
                    <div class="rows">
                        <label>商品名称：</label>
                        <span class="input">
                            <label>${goodsSnapshot.name}</label>
                        </span>
                        <div class="clear"></div>
                    </div>
                    <div class="rows">
                        <label>封面图片：</label>
                        <span class="input">
                            <img src="${RESOURCE_LOCAL_URL}${goodsSnapshot.coverPicOne}" style="width: 150px;height: 150px;;">
                        </span>
                        <div class="clear"></div>
                    </div>
                    <div class="rows">
					...
					<c:if test="${reGoodsOfTeamMall.isRestrict==true}">
					 	 <div class="rows">
                            <label>限购数量：</label>
                            <span class="input">
                                <lable>${reGoodsOfTeamMall.restrictNum}</lable>
                            </span>
                            <div class="clear"></div>
                        </div>
					</c:if>
                    <c:if test="${reGoodsOfTeamMall.reGoodsOfSellerMall.isNoStandard}">
                        <div class="rows">
                            <label>发布数量：</label>
                            <span class="input">
                                <lable>${reGoodsOfTeamMall.releaseNum}</lable>
                            </span>
                            <div class="clear"></div>
                        </div>
                         <div class="rows">
                            <label>市场价：</label>
						<span class="input">
                            <label>${reGoodsOfTeamMall.reGoodsOfSellerMall.noStandardPrice}</label>
			 			</span>
                            <div class="clear"></div>
                    	</div>
                        <div class="rows">
                            <label>拼团优惠价：</label>
                            <span class="input">
                                <label>${reGoodsOfTeamMall.discountPrice}</label>
                            </span>
                            <div class="clear"></div>
                        </div>
                    </c:if>
                            ...
                            <div class="tableArea" style="display: none;">
                                <table id="table">
                                    ...
                                    <tr id="tableProperty" class="headLine">
                                        ${reGoodsOfTeamMallTableHead}
                                        <th>操作</th>
                                    </tr>
                                    </thead>
                                    <tbody id="newbody">
                                        ${reGoodsOfTeamMallTableLine}
                                    </tbody>
                                </table>
                            </div>
                            <script>
                    ...
                    <div class="rows">
                        <label></label>
						<span class="input">
							<input type="button" value="审核通过" onclick="pass(${reGoodsOfTeamMall.id},true);" style="color:#fff; font-size:1.0rem; line-height:2.5rem; width:6.0rem; background:#0e93d8; border-radius:3px; cursor:pointer;"/>
							<input type="button" value="审核不通过" onclick="pass(${reGoodsOfTeamMall.id},false);" style="color:#fff; font-size:1.0rem; line-height:2.5rem; width:6.0rem; background:#0e93d8; border-radius:3px; cursor:pointer;"/>
						</span>
                        <div class="clear"></div>
                        <script>
                            function pass(goodsId, isPass) {
                                var d = $(".checkDesc").val();
                                $.post("checkOfTeamMall", {"goodsId": goodsId, "checkDesc": d, "isPass": isPass}, function (data) {
                                    alert(data.message);
                                    location.href = "listOfTeamMall";
                                });
                            }
                        </script>
                    </div>
