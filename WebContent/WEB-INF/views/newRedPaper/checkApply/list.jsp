...
function edit(id){
	window.location.href="<%=basePath%>newRedPaper/checkApply/edit?id="+id; 
}
function history(id){
					...
					<tbody>
						<c:forEach items="${requestScope.redpaperList }" var="data">
							<tr>
								...
								<td>${data.id}</td>
								<td>${data.adminUser.loginname}</td>
            					<td><fmt:formatNumber  value="${data.positons}" type="number" />元</td>
            					<td><fmt:formatDate value="${data.createTime}" type="date" pattern="yyyy-MM-dd"/></td>
            					<td>
            						<c:if test="${data.status==0}"><font>未审核</font></c:if>
            						<c:if test="${data.status==1}"><font color="green">审核通过</font></c:if>
            						<c:if test="${data.status==2}"><font color="red">审核不通过</font></c:if>
            					</td>
            					<td>
            					<div>
									...
									<c:if test="${data.status!=0}">已审核</c:if>
								</div>
            					</td>
							</tr>
						</c:forEach>
