...
<body>
	...
	<div class="main" style="height:auto;">
		...
		<div class="div_search">
			<form action="<%=basePath%>taoke/pids/findPidByCondition"
				...
				<tbody>
					...
					<c:forEach items="${list}" var="pid">
						<tr>
							...
							<td>${pid.remark}</td>
							<td>
								<c:if test="${pid.level==1}">运营商</c:if> 
	...
	<script type="text/javascript">
		...
		function upgradeCareer(id, level) {
			...
			var level=$("#level").val();
					...
					$.ajax({
						async : false,
						url : '${BASEPATH_IN_SESSION}taoke/pids/upgradeCareer?id='
								+ id + "&level=" + level,
						dataType : "json",
						success : function(data) {
							if (data.flag = "1") {
								alert("升级成功");
								location.href = "${BASEPATH_IN_SESSION}taoke/pids/findPidByCondition?pId="+pId+"&userNames="+userNames+"&adminNames="+adminNames+"&level="+level;
