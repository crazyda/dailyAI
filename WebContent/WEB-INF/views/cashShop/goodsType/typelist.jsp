		...
		<div class="main" style="height:auto;">
			...
			<div class="div_search">
			   <form action="<%=basePath%>cashtype/typelist.action" method="post">
				  ...
				  所属模块:	<select name="modelId">
				  			<!-- <option value="">全部模块</option> -->
					  		<c:forEach items="${modules }" var="m">
					  			<option value="${m.key }" <c:if test="${ m.key == curruModelId}" >selected="selected"</c:if> >${m.value }</option>
					  		</c:forEach>
				  		</select>   
					...
					<tbody>
						<c:forEach items="${typeList }" var="type">
							<tr>
								...
								<td>${type.id }</td>
								<%-- <td>${modules[type.modelId] }</td> --%>
								<td>${type.name }</td>
								<td>${type.level }</td>
								<td>
								<c:if test="${type.commodityType!=null }">
								${type.commodityType.name }
								</c:if>
								</td>
								<td><img src="${RESOURCE_LOCAL_URL }${type.imgUrl }" style="width:80px;height:80px;"/></td>
								<td>${type.isLocal==true?"是":"否"}</td>
								<td>
								<div align="center">
