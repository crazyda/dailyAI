			...
			<div class="div_search">
			   ...
			   <c:forEach  items="${leveltable }" var="levelset" >
				    ...
				    <c:if test="${aus.level==75 or aus.level==95}">
				    	积分池总积分:${aus.score } ,奖金池:
				    </c:if>
				    <c:if test="${aus.level==65 }">
				    	<input type="hidden" name="user_num" value="1"/>
				    	
				    		<c:if test="${backState==null||backState==10 }">
				            <input type="button" value="用户数" class="button"/>
				             <input  value="商家数" class="button" type="button"onclick="javascript:window.location.href='list?backState=20';"/>
				            </c:if>
				    	...
				    	积分池总积分:${aus.parentAdminUser.score } ,奖金池:
				    </c:if>
			    </form>
			</div>
			<!--end of header-->
			<div class="pageColumn" id="pageColumn">
				<table class="list" style="width:98%;margin:0 auto;">
					<thead>
						...
						<c:if test="${aus.level==65 ||  aus.level==95}">
							<th width="8%">分佣</th>
