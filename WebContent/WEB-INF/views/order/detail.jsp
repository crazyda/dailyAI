...
<meta charset="utf-8">
<title>待发货</title>
<link type="text/css" href="<%=basePath %>css/order/child-page.css" rel="stylesheet">
</head>
            ...
            <tbody>
               ...
               <c:forEach  items="${orderMap['items']}" var="item" >
                <tr>
                    ...
                    <c:if test="${item.status==10 }">待确认</c:if>
                    <c:if test="${item.status==20 }">待发货</c:if>
                    <c:if test="${item.status==30 }">待兑换</c:if>
                    </td>
