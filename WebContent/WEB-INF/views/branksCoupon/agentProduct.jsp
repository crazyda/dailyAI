			...
			<div class="div_search">
			   <form action="BranksCoupon/getProductToAgent" method="post">
				...
				<c:if test="${adminUser.level >= 95 }">
			  		归属代理:	<select name="dlId">
			  				<option value="-1">-请选择-</option>
				  		<c:forEach items="${adminUsers }" var="a">
				  			<option value="${a.id }" <c:if test="${ a.id == dlId}" >selected="selected"</c:if> >${a.username }</option>
				  		</c:forEach>
			  		</select>  
			...
			<div class="pageColumn">
				...
				<table class="list" style="width:98%;margin:0 auto;">
					<thead>
						...
						<th width="8%" style="height:50px;">归属银行</th>
						<th width="5%">产品名称</th>
						<th width="7%">可获积分</th>
						<th width="5%">收分用户</th>
						<th width="5%">基本操作</th>
						...
						<c:forEach items="${agentProducts }" var="type">
							<tr>
								...
								<td>
									...
									<c:if test="${adminUser.level == 65 }">
										<div align="center">
											<span class="STYLE4">
											<span style="cursor: pointer;"onclick="add('${type.id }');">添加到我的仓库</span>
											</span>
										</div>
